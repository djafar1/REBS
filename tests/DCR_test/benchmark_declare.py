...
import pandas as pd
import time
from pm4py.discovery import discover_declare
from pm4py.conformance import conformance_declare
...
def benchmark_conformance_run_time_pdc(test_path, training_logs, gt_truth_log):
    ...
    for file in os.listdir(training_log_path):
        ...
        for gt_file in os.listdir(gt_path):
            ...
            res = conformance_declare(test_log, model)
            ...
            box_fitness[size].append((1 - len(collect)/size))
...
def benchmark_conformance_run_time_sepsis(test_path):
    ...
    #discover model
    graph , _ = discover_declare(training_log)
    ...
    for file in os.listdir(test_path):
        ...
        for i in temp:
            avg_trace_len += len(i)
        avg_trace_len = avg_trace_len/len(temp)

        temp={"file": str(file),"total_deviations": len(collect), "time": end, "avg_trace": avg_trace_len, "log_size": len(test_log)}
        write_csv(temp, file)
...
if __name__ == "__main__":
    ...
    if "sepsis" in enabled_testing:
        ...
        print([random.randrange(20, 61, 5) for _ in range(100)])
        configuration_trace_len = [random.randrange(20, 61, 5) for _ in range(20)]
        max_trace = 1050
