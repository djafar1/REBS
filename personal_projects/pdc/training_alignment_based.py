...
def classification_result(trace_to_test, only_file, logs_hists_for_events, base_log, test_log, ground_truths):
    ...
    for event in test_case['concept:name'].unique():
        ...
        if len(test_arr) > 0 and len(base_arr) > 0:
            ...
            # print(f'[vals] train: {mu} test: {test_pos} base: {base_pos}')
            # print(f'[test] {mu-2*sigma} < {test_pos} < {mu+2*sigma}')
            if test_pos != base_pos:
                if abs(mu-test_pos) < abs(mu-base_pos):
                    sum_of_test += 1
                else:
                    sum_of_base += 1
                # if (mu-scale*sigma) < test_pos < (mu+scale*sigma):
                #     sum_of_test += 1
                # print(f'[base] {mu-2*sigma} < {base_pos} < {mu+2*sigma}')
                # if (mu-scale*sigma) < base_pos < (mu+scale*sigma):
                #     sum_of_base += 1
