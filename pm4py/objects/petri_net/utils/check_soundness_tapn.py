...
from pm4py.objects.petri_net.timed_arc_net import obj


def check_source_and_sink_reachability(net, initial_sources, unmarked_sinks):
    """
    Checks reachability of the source (sources can be any) and the (sink must not be certain) places
    from all simulation nodes (places/transitions)
    of the Petri net
    ...
    graph, unique_source_corr, unique_sink_corr, inv_dictionary = create_networkx_undirected_graph(net, initial_sources,
                                                                                                   unmarked_sinks)
...
def check_source_place_presence(net):
    """
    Check if there is a subset of marked places.
    Relate to the soundness of DCR Graphs
    Parameters
    -------------
    net
        Petri net
...
def check_sink_place_presence(net):
    """
    Check if there are no tokens in the pending included places
    Check if tokens can be removed from the initially pending places.
    Relate to the soundness of DCR Graphs
    Parameters
    -------------
    net
...
def check_wfnet(net):
    """
    Check if the Petri net is a declarative net
    Has the pending places
    Parameters
    ------------
    net
        Petri net
...
def check_easy_soundness_of_wfnet(net):
    ...
    Base it on the dcr graph and bisimalarity
    Parameters
    -------------
    net
        Petri net
