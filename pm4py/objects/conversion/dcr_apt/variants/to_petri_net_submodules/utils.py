from pm4py.objects.conversion.dcr_apt.variants.to_petri_net_submodules.single_relations_dcr2apt import ArcType
...
def create_event_pattern_transitions_and_arcs(apt, event, helper_struct, mapping_exceptions):
    '''
    ...
    for t_name in helper_struct[event]['t_types']:  # ['event','init','initpend','pend']:
        ...
        if event in mapping_exceptions.self_exceptions[frozenset([Relations.E.value, Relations.R.value])]:
            apt[(pend_exc_place, t)] = ArcType.TtoP

        apt[(inc_place, t)] = ArcType.PtoT
        ...
        if not ((event in mapping_exceptions.self_exceptions[Relations.E.value]) or (
            ...
            apt[(inc_place, t)] = ArcType.TtoP

        # this if statement handles self response
        if event in mapping_exceptions.self_exceptions[Relations.R.value]:
            apt[(pend_place, t)] = ArcType.TtoP

        if t_name.__contains__('init'):
            apt[(exec_place, t)] = ArcType.TtoPandInhib
        else:
            apt[(exec_place, t)] = ArcType.Both

        if t_name.__contains__('pend'):
            apt[(pend_place, t)] = ArcType.PtoT
        else:
            apt[(pend_place, t)] = ArcType.Inhib
        ts.append(t)
