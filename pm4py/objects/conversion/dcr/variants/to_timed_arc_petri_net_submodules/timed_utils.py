...
def map_existing_transitions_of_copy_0(delta, copy_0, t, tapn) -> (TimedArcNet, TimedArcNet.Transition):
    ...
    for arc in in_arcs:
        ...
        s_to_t = pn_utils.add_arc_from_to_with_check(source, t, tapn, type=type, with_check=True)
    ...
    for arc in out_arcs:
        target = arc.target
        type = arc.properties['arctype'] if 'arctype' in arc.properties else None
        t_to_t = pn_utils.add_arc_from_to_with_check(t, target, tapn, type=type, with_check=True)
