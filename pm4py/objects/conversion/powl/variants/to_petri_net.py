...
from pm4py.objects.petri_net.utils.petri_utils import add_arc_from_to_with_check, remove_place
...
def recursively_add_tree(powl, net, initial_entity_subtree, final_entity_subtree, counts, rec_depth,
    ...
    if isinstance(powl, Transition):
        ...
        add_arc_from_to_with_check(initial_place, tau_split, net)
        tau_join = pt_to_pn.get_new_hidden_trans(counts, type_trans="tauJoin")
        net.transitions.add(tau_join)
        add_arc_from_to_with_check(tau_join, final_place, net)
        ...
        for subtree in tree_children:
            ...
            if subtree in start_nodes:
                ...
                add_arc_from_to_with_check(tau_split, i_place, net)

                add_arc_from_to_with_check(i_place, i_trans, net)
            ...
            if subtree in end_nodes:
                ...
                add_arc_from_to_with_check(f_trans, f_place, net)
                add_arc_from_to_with_check(f_place, tau_join, net)
        ...
        for i, j in product(n, n):
            if transitive_reduction.is_edge_id(i, j):
                ...
                add_arc_from_to_with_check(final_trans[i], new_place, net)
                add_arc_from_to_with_check(new_place, init_trans[j], net)
